    ...
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            while ($pet = $result->fetch_assoc()) {
                ...
                echo '<div class="card" style="display: flex; align-items: center; margin-bottom: 20px;">';
                
                // Image section
                if (!empty($pet['image_url'])) {
                    echo '<div style="flex: 1; margin-right: 20px;">';
                    echo '<img src="' . htmlspecialchars($pet['image_url']) . '" alt="' . htmlspecialchars($pet['petname']) . '" style="max-width: 150px; height: auto;">';
                    echo '</div>';
                } else {
                    echo '<div style="flex: 1; margin-right: 20px;">';
